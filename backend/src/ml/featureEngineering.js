...
function skillOverlap(freelancerSkills, jobSkills) {
  const fSet = new Set((freelancerSkills || []).map((s) => String(s).toLowerCase()));
  const jSkills = (jobSkills || []).map((s) => String(s).toLowerCase());
  if (!jSkills.length) return 0.5;
  const overlap = jSkills.filter((s) => fSet.has(s)).length;
...
function budgetFit(jobBudget, avgBid) {
...
function buildFeatureVector(freelancer, job, stats) {
  const features = {
...
async function loadFreelancerStats(publicKey) {
...
async function buildPairFeatures(freelancerAddress, jobRow) {
...
async function buildBatchFeatures(freelancerAddress, jobRows) {
  const [freelancer, stats] = await Promise.all([
    loadFreelancerProfile(freelancerAddress),
    loadFreelancerStats(freelancerAddress),
  ]);
...
async function buildFreelancerBatchFeatures(freelancerRows, jobRow) {
  const clientRating = jobRow.client_rating;

  const results = await Promise.all(
    freelancerRows.map(async (freelancer) => {
